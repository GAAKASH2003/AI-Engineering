...
def optimize(optimizer,func,grad_func,start,steps):
    params=list(start)
    history=[params[:]]
...
def find_convergence_step(history,func,threshold=1e4):
...
def demo_learning_rate_effect():
    ...
    print(f"{'=' * 60}")
    start = [-1.0, 1.0]
    rates = [0.0001, 0.0005, 0.001, 0.005,0.01,0.00001]
    ...
    for lr in rates:
        ...
        history = optimize(gd, rosenbrock, rosenbrock_gradient, start, 10000)
        if history is None:
            return []
        final = history[-1]
        loss = rosenbrock(final)
...
def demo_saddle_point():
    ...
    configs = [
        ("Gradient Descent", GradientDescent(lr=0.05)),
        ("SGD + Momentum",   SGDMomentum(lr=0.05, momentum=0.95)),
        ("Adam",             Adam(lr=0.05)),
    ]
